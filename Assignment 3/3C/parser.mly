...
%token PLUS_INTEGER MINUS_INTEGER TIMES_INTEGER DIVIDE_INTEGER SQUARE_ROOT
%token ADD_VECTOR SCALAR_MULTIPLY ADD_MATRIX SCALAR_MATRIX_MULTIPLY MATRIX_MULTIPLY VECTOR_MULTIPLY
...
%token INVERSE INVERSE_VECTOR INVERSE_MATRIX DETERMINANT TRANSPOSE MINOR
...
loops: 
  | FOR LEFT_PAREN VAR ASSIGN express TO express RIGHT_PAREN DO LEFT_BRACE lines RIGHT_BRACE { For($3, $5, $7, $11) }
  | WHILE express DO LEFT_BRACE lines RIGHT_BRACE { While($2, $5) } 

print:
  | PRINT LEFT_PAREN VAR RIGHT_PAREN { Print($3) } 
  | ERROR { Error }
;

input:
  | VAR ASSIGN INPUT LEFT_PAREN RIGHT_PAREN { Input($1) }
  | VAR ASSIGN INPUT LEFT_PAREN FILENAME RIGHT_PAREN { Input_File($1, $5) }
;

if_stmt:
  | IF LEFT_PAREN express RIGHT_PAREN THEN LEFT_BRACE lines RIGHT_BRACE ELSE LEFT_BRACE lines RIGHT_BRACE { IfElse ($3, $7, $11) }
...
create:
  | TYPE_BOOL VAR { Create_bool($2) }
  | TYPE_INTEGER VAR { Create_int($2) }
  | TYPE_SCALAR VAR { Create_float($2) }
  | TYPE_VECTORFLOAT VAR INT { Create_vecfloat($2, $3) }
  | TYPE_MATRIXINT VAR INT INT { Create_matint($2, $3, $4) }
  | TYPE_VECTORINT VAR INT { Create_vecint($2, $3) }
  | TYPE_MATRIXFLOAT VAR INT INT { Create_matfloat($2, $3, $4) }
...
express:
  ...
  | SQUARE_ROOT LEFT_PAREN express RIGHT_PAREN { UnOp_Ad(SquareRoot, $3) }
  | VECTOR_MULTIPLY LEFT_PAREN express COMMA express RIGHT_PAREN { BinOp_Ad(VectorMultiply, $3, $5) }
  | VAR LEFT_BRACKET express RIGHT_BRACKET ASSIGN express { AssignVec($1, $3, $6) }
  | VAR LEFT_BRACKET express RIGHT_BRACKET LEFT_BRACKET express RIGHT_BRACKET ASSIGN express { AssignMat($1, $3, $6, $9) }
  | indexing { $1 }
  | input { $1 }
;
...
matrix_oper:
  ...
  | MINOR LEFT_PAREN express COMMA INT COMMA INT RIGHT_PAREN { Minor($3, $5, $7) }
...
indexing:
  | VAR LEFT_BRACKET express RIGHT_BRACKET { IndexVec($1, $3) }
  | VAR LEFT_BRACKET express RIGHT_BRACKET LEFT_BRACKET express RIGHT_BRACKET { IndexMat($1, $3, $6) }
